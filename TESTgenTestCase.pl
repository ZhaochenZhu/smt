...
while (<$in>) {
    ...
    if ($line =~ /\.SUBCKT (\S+)/) {
        ...
        # mapping PMOS/NMOS (non-power delivery) pins to its count (occurance)
		if($instType eq "PMOS"){
            # PMOS --> count non-power delivery pins
            # source
			if(!exists($h_netcnt{"P_$h_nets{$net_s}"})){
				$h_netcnt{"P_$h_nets{$net_s}"} = 1;
				print "Source:$net_s P_$h_nets{$net_s} $h_netcnt{\"P_$h_nets{$net_s}\"}\n";
			}
            # source is not power delivery
            ...
            # NMOS --> count non-power delivery pins
            # source
