...
# 2022-07-17 GEAR RATIO: Allow specification of number of tracks on each metal layer
### Pre-processing ########################################################
my $k=1;
my $ARGC        = @ARGV;
my $workdir     = getcwd();
my $outdir      = "$workdir/pinLayouts_cfet_v2.2";
my $infile      = "";
...
my @StdCells = ("NAND2x1_ASAP7_75t_R");
...
my @mapTrackH; # Horizontal Track for each metal layer
=begin
sub getTrack{
	my $track = @_[0];
	my $nTrack = -1;
...
=begin
    Assume 3 Fins per FET, Every set of Fins will be mapped to 2 set of nanosheets.
    Number of set of Nanosheet determines the width of the FET
=cut

	my $nfin = @_[0];
    my $nNS = 0;
	my $fin_perFET = 3;     # assume 3 fins per fet
	my $multiple = int($nfin/$fin_perFET);      # how many set of fins given the number of fins
	...
	#for my $i(0 .. $#mapNS){
        ...
        #        }
        #}
	print "Convert nfinger: $multiple => nNS: $nNS\n";
	return $nNS;
}
...
# YW UPDATE: now accepts multiple inputs for layers
if ($ARGC < 2) {
    print "\n*** Error:: Wrong CMD";
    print "\n   [USAGE]: ./PL_FILE [inputfile_spfile] [numTrackV Offset]\n\n";
    exit(-1);
...
my $numTrackH = 6; # 4 routing tracks; 2 power/ground tracks, NEVER USED
#my $numClip = 2; # PMOS and NMOS region in FinFET
my $numClip = 1; # PMOS and NMOS region for CFET, NEVER USED
...
while (<$in>) {
    ...
    if ($line =~ /\.SUBCKT (\S+)/) {
		...
		@ext_pins = ();
		@ext_powerpins = ();
		@pins = ();             # @pins = (pinID, netID, instID, pinName, pinDirection, pinLength)
		@nets = ();             # @nets = (netID, N-pinNet, PinList)
		@inst = ();             # @inst = (instID, Type, Width)
		%h_inst = ();           # map instID ==> index
		%h_nets = ();           # map source/drain/gate ==> index
		%h_nets_source = ();
		%h_name_nets = ();      # map index ==> source/drain/gate (reverse linking h_nets)
		%h_pintype = ();
		%h_netcnt = ();
        ...
        # mapping PMOS/NMOS SOURCE/DRAIN pins to its count (occurance)
        # VDD/VSS only gets count once
        # PMOS/NMOS pin are different and are counted separately(e.g. P_(NetID) and N_(NetID))
		if($instType eq "PMOS"){
            # source
			if(!exists($h_netcnt{"P_$h_nets{$net_s}"})){
				$h_netcnt{"P_$h_nets{$net_s}"} = 1;
				print "Source:$net_s P_$h_nets{$net_s} $h_netcnt{\"P_$h_nets{$net_s}\"}\n";
			}
            # source is not power delivery, count non-power delivery pins
			...
			if(!exists($h_netcnt{"P_$h_nets{$net_d}"})){
				...
				print "Drain:$net_d P_$h_nets{$net_d} $h_netcnt{\"P_$h_nets{$net_d}\"}\n";
			}
            # drain is not power delivery, count non-power delivery pins
			...
			if(!exists($h_netcnt{"N_$h_nets{$net_s}"})){
				...
				print "Source:$net_s N_$h_nets{$net_s} $h_netcnt{\"N_$h_nets{$net_s}\"}\n";
			}
            # source is not power delivery, count non-power delivery pins
			elsif ($net_s ne "VDD" && $net_s ne "VSS" ) {
				$h_netcnt{"N_$h_nets{$net_s}"} += 1;
			...
			if(!exists($h_netcnt{"N_$h_nets{$net_d}"})){
				...
				print "Drain:$net_d N_$h_nets{$net_d} $h_netcnt{\"N_$h_nets{$net_d}\"}\n";
			}
            # drain is not power delivery, count non-power delivery pins
			elsif ($net_d ne "VDD" && $net_d ne "VSS" ) {
				$h_netcnt{"N_$h_nets{$net_d}"} += 1;
        ...
        # print "h_netcnt:", Dumper(\%h_netcnt);
        # exit(0);

        ######################################## SOURCE ############################################
		my $isSource = 0;
        # if source not exists in net_source 
		if(!exists($h_nets_source{$net_s})){
			$isSource = 1; # if never encountered before, it is a nets_source
			$h_nets_source{$net_s} = $idx_pin;
		}

		# @pins = (pinID, netID, instID, pinName, pinDirection, pinLength)
        print "pushing ", " pinID: ", $idx_pin, " netID: ", (exists($h_nets{$net_s})?$h_nets{$net_s}:$idx_net), " instID: ", $instID, " pinName: S", " pinDirection: ", ($isSource==1?"s":"t"), " pinLength: ", $inst[$h_inst{$1}][2], "\n";
		push(@pins, [($idx_pin, (exists($h_nets{$net_s})?$h_nets{$net_s}:$idx_net), $instID, "S", ($isSource==1?"s":"t"), $inst[$h_inst{$1}][2] )]);
		# update net info
		$nets[$h_nets{$net_s}][1] = $nets[$h_nets{$net_s}][1] + 1;                                                                                                                  # increase net N pinNet
        print(" SOURCE adding ", $isSource==1?(($nets[$h_nets{$net_s}][2] eq ""?"":" ").$nets[$h_nets{$net_s}][2]."pin$idx_pin") : ("pin$idx_pin ".$nets[$h_nets{$net_s}][2]), " to ", $net_s, "\n");
		$nets[$h_nets{$net_s}][2] = $isSource==1?(($nets[$h_nets{$net_s}][2] eq ""?"":" ").$nets[$h_nets{$net_s}][2]."pin$idx_pin") : ("pin$idx_pin ".$nets[$h_nets{$net_s}][2]);   # add to net PinList

        # update SOURCE pin index
		my $idx_pin_s = $idx_pin;
		$idx_pin++;

        ######################################## GATE ############################################
		$isSource = 0;
        # if gate not exists in net_gate
		if(!exists($h_nets_source{$net_g})){
			$isSource = 1; # if never encountered before, it is a nets_source
			$h_nets_source{$net_g} = $idx_pin;
		}
		# @pins = (pinID, netID, instID, pinName, pinDirection, pinLength)
        print "pushing ", " pinID: ", $idx_pin, " netID: ", (exists($h_nets{$net_g})?$h_nets{$net_g}:$idx_net), " instID: ", $instID, " pinName: G", " pinDirection: ", ($isSource==1?"s":"t"), " pinLength: ", $inst[$h_inst{$1}][2], "\n";
		push(@pins, [($idx_pin, (exists($h_nets{$net_g})?$h_nets{$net_g}:$idx_net), $instID, "G", ($isSource==1?"s":"t"), $inst[$h_inst{$1}][2] )]);
		# update net info
		$nets[$h_nets{$net_g}][1] = $nets[$h_nets{$net_g}][1] + 1;                                                                                                                  # increase net N pinNet
        print(" GATE adding ", $isSource==1?(($nets[$h_nets{$net_g}][2] eq ""?"":" ").$nets[$h_nets{$net_g}][2]."pin$idx_pin"):("pin$idx_pin ".$nets[$h_nets{$net_g}][2]), " to ", $net_g, "\n");
		$nets[$h_nets{$net_g}][2] = $isSource==1?(($nets[$h_nets{$net_g}][2] eq ""?"":" ").$nets[$h_nets{$net_g}][2]."pin$idx_pin"):("pin$idx_pin ".$nets[$h_nets{$net_g}][2]);     # add to net PinList
		$idx_pin++;
		...
		# @pins = (pinID, netID, instID, pinName, pinDirection, pinLength)
        print "pushing ", " pinID: ", $idx_pin, " netID: ", (exists($h_nets{$net_d})?$h_nets{$net_d}:$idx_net), " instID: ", $instID, " pinName: D", " pinDirection: ", ($isSource==1?"s":"t"), " pinLength: ", $inst[$h_inst{$1}][2], "\n";
		push(@pins, [($idx_pin, (exists($h_nets{$net_d})?$h_nets{$net_d}:$idx_net), $instID, "D", ($isSource==1?"s":"t"), $inst[$h_inst{$1}][2] )]);
		# update net info
		$nets[$h_nets{$net_d}][1] = $nets[$h_nets{$net_d}][1] + 1;                                                                                                                  # increase net N pinNet
        print(" DRAIN adding ", $isSource==1?(($nets[$h_nets{$net_d}][2] eq ""?"":" ").$nets[$h_nets{$net_d}][2]."pin$idx_pin"):("pin$idx_pin ".$nets[$h_nets{$net_d}][2]), " to ", $net_d, "\n");
		$nets[$h_nets{$net_d}][2] = $isSource==1?(($nets[$h_nets{$net_d}][2] eq ""?"":" ").$nets[$h_nets{$net_d}][2]."pin$idx_pin"):("pin$idx_pin ".$nets[$h_nets{$net_d}][2]);     # add to net PinList

        # Double mapping SOURCE --> DRAIN and DRAIN --> SOURCE
        ...
        # print "pins:", Dumper(\@pins);
        
	
	}
    elsif ($subckt_flag == 1 and $line =~ /\.ENDS/) {
        $subckt_flag = 0; # reset back to 0

		foreach my $key(keys %h_netcnt){
			print "this is used ", "$key => $h_netcnt{$key}\n";
		}
        ...
        # print "pins_before:", Dumper(\@pins);
        # Changed the original pin type???
		for my $i(0 .. $#pins){

            # only cares about SOURCE/DRAIN
			if($pins[$i][2] ne "ext" && $pins[$i][3] ne "G" && !exists($h_touchedpin{$pins[$i][0]})){
                ...
                # NET that the SOURCE/DRAIN pins belongs to and its occurance
				my $numCon_cur = $h_netcnt{"$prefix_type\_$pins[$i][1]"};
                # NET that the SOURCE/DRAIN pins Match belongs to and its occurance
				my $numCon_match = $h_netcnt{"$prefix_type\_$pins[$h_pinmatch{$pins[$i][0]}][1]"};

                print("*****************what is this shit ", "$prefix_type\_$pins[$i][1]", " ", "$prefix_type\_$pins[$h_pinmatch{$pins[$i][0]}][1]", "\n");
                ...
                # if PinName is SOURCE and Current Net SOURCE Occur < Matched Net DRAIN Occur, flip the DRAIN/SOURCE
				if($pins[$i][3] eq "S" && $numCon_cur < $numCon_match){
					$pins[$i][3] = "D";
					$pins[$h_pinmatch{$pins[$i][0]}][3] = "S";
				}
                # if PinName is DRAIN and Current Net DRAIN Occur > Matched Net SOURCE Occur, flip the DRAIN/SOURCE
				elsif($pins[$i][3] eq "D" && $numCon_cur > $numCon_match){
					$pins[$i][3] = "S";
					$pins[$h_pinmatch{$pins[$i][0]}][3] = "D";
				}
                # if PinName is DRAIN and Current Net DRAIN Occur == Matched Net SOURCE Occur
				elsif($numCon_cur == $numCon_match){
                    # if pin belongs to a ext power net, current pin is a SOURCE, matched pin is DRAIN
					if($h_pintype{$h_name_nets{$pins[$i][1]}} eq "P"){
						$pins[$i][3] = "S";
						$pins[$h_pinmatch{$pins[$i][0]}][3] = "D";
					}
                    # if matched pin belongs to a ext power net, , matched pin is a SOURCE, current pin is DRAIN
                ...
                # for each DRAIN/SOURCE pairs, only visit once
        ...
        # print "pins_after:", Dumper(\@pins);

        # print "h_touchedpin:", Dumper(\%h_touchedpin);
        # exit(0);
		...
		my $minWidth = $sizeNMOS>$sizePMOS?$sizeNMOS:$sizePMOS; # YW: it can be more?

		# Output file
		### Write PinLayout
		print "a   Write PinLayout\n";
		# Routing Clip only corresponds to one of the tracks
		...
		if (@mapTrackH) {
			for my $i(0 .. $#mapTrackH){
				print "a     Tracks for Metal ", $i , " = $mapTrackH[$i]\n";
			}
		}
		...
		if (@mapTrackH) {
			for my $i(0 .. $#mapTrackH){
				print  $out "a     Tracks for Metal ", $i , " = $mapTrackH[$i]\n";
			}
		}
    ...
    # YW TESTING: if print data structure
    if (0) {
        if (%h_nets) {
            print("h_nets: \n");
            print Dumper(\%h_nets);
        ...
        if (%h_name_nets) {
            print("h_name_nets: \n");
            print Dumper(\%h_name_nets);
        }

        if (%h_netcnt) {
            print("h_netcnt: \n");
            print Dumper(\%h_netcnt);
        }
        
        if (%h_nets_source) {
            print("h_nets_source: \n");
            print Dumper(\%h_nets_source);
        }
        ...
        if (%h_pintype) {
            print("h_pintype: \n");
            print Dumper(\%h_pintype);
        }
        
        if (%h_pinmatch) {
            print("h_pinmatch: \n");
            print Dumper(\%h_pinmatch);
        }

        if (@ext_pins) {
            print("ext_pins: \n");
            print Dumper(\@ext_pins);
        }
        
        if (@ext_powerpins) {
            print("ext_powerpins: \n");
            print Dumper(\@ext_powerpins);
        }
    }
